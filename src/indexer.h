...
void MultiIndexer<Record>::SerializeMultiIndexFiles() {
  ...
  ofstream cell_edges2(string(files_prefix_ + "_cell_edges.txt").c_str());
  boost::archive::text_oarchive txt_cell_edges(cell_edges2);
  txt_cell_edges << multiindex_.cell_edges;
  ...
  ofstream multi_array2(string(files_prefix_ + "_multi_array.txt").c_str(), ios::binary);
  boost::archive::text_oarchive txt_multi_array(multi_array2);
  txt_multi_array << multiindex_.multiindex;
...
void MultiIndexer<Record>::ConvertPointsInCellsCountToCellEdges() {
	//ofstream m_table("multitable.txt");
	//ofstream edges_tmp("edges.txt");
	...
	//m_table << point_in_cells_count_.table[0] << endl;
	//edges_tmp << multiindex_.cell_edges.table[0] << endl;
	for(int global_index = 1; global_index < point_in_cells_count_.table.size(); ++global_index) {
		//m_table << point_in_cells_count_.table[global_index] << endl;
		...
		//edges_tmp << multiindex_.cell_edges.table[global_index] << endl;
	}
...
// For a fea vector in db, it's coarse quantizations are id1, id2
// global_id = id1 * 256 + id2 (totally 256*256 = 65566 global ids)
// then point_in_cells_count_[global_id] += 1;
template<class Record>
